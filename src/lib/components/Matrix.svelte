...
<script lang="ts">
    ...
    $: columns = (
        $spec.measures ? $spec.measures.map((m: Measure) => toStatistic(matrix, m)) : []
    ) as Array<Statistic>;
...
<div class="box hover">
    {#if $currentCell}
        <span class="highlight">{$currentCell[0].data.name.substring(0, 21)}</span> was labeled as
        <span class="highlight">{$currentCell[1].data.name.substring(0, 21)}</span>
...
<svg width={totalExtent} {height}>
    <g transform="translate({padding + axisTitle},{padding + axisTitle})">
        {#if $spec.filter?.length > 0}
            <text class="breadcrumb" on:click={clearFilter} on:keydown={clearFilter} y="120"
                >{'< go back'}</text
            >
        {/if}

        <g transform="translate(0,0)">
            <text class="axisTitle"
                >{$spec.normalization === 'total'
                    ? 'Counts'
                    : `${$spec.normalization} Probabilities`}</text
            >
        ...
        <text
            text-anchor="middle"
            class="axisTitle"
            transform="translate({hierarchyExtent + confusionsExtent / 2},0)">Observed</text
        >
        {#each layout as { node: predict, pos: predictPos }}
            <g
                transform="translate({predictPos[1] * cellSize +
                    cellSize / 2 +
                    hierarchyExtent},{hierarchyExtent - predictPos[0] * hierarchyIndent})"
            >
        ...
        <text
            text-anchor="middle"
            class="axisTitle"
            transform="translate({-axisTitle / 2},{hierarchyExtent +
                confusionsExtent / 2}) rotate(-90)">Actual</text
        >
        <g transform="translate(0,{hierarchyExtent})">
            {#each layout as { node: actual, pos: actualPos, span }}
                <g
                    transform="translate({actualPos[0] * hierarchyIndent},{actualPos[1] * cellSize +
                        cellSize / 2})"
                >
                    {#if span > 0}
                        <line
                            ...
                            x2="0.3em"
                            y1={cellSize / 2}
                            y2={span * cellSize + cellSize / 2}
                            stroke={actual.children.find(
                                (n) => $currentCell && n.data.id === $currentCell[0].data.id
                            )
                                ? 'black'
                                : '#cccccc'}
                        />
                ...
                <g transform="translate(0,{actualPos[1] * cellSize})">
                    {#each layout as { node: predict, pos: predictPos }}
                        {#if !isFrontier(predict) || !isFrontier(actual)}
                            <CellBlank x={predictPos[1] * cellSize} {actual} {predict} {cellSize} />
                        {:else if matrix.frequency(actual, predict) === 0}
                            <CellZero x={predictPos[1] * cellSize} {actual} {predict} {cellSize} />
        ...
        <g
            transform="translate({hierarchyExtent +
                confusionsExtent +
                statisticsPadding},{hierarchyExtent})"
        >
